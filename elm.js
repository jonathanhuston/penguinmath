...
var author$project$PenguinMath$getNextPage = function (model) {
	return (_Utils_cmp(model.count, model.quiz.total) < 0) ? _Utils_update(
		model,
		{myAnswer: '', page: author$project$PenguinMath$AskQuestion}) : ((_Utils_cmp(model.right, model.quiz.goal) > -1) ? _Utils_update(
		model,
...
var author$project$PenguinMath$rightOrWrong = function (model) {
	...
	return _Utils_eq(model.myAnswer, answer) ? (model.lastWrong ? _Utils_update(
		model,
...
var author$project$PenguinMath$displayCaption = function (model) {
	...
	var question = function () {
		var _n1 = A2(elm$core$Array$get, model.count, model.quiz.questions);
		if (_n1.$ === 'Just') {
			var q = _n1.a;
			return q;
		} else {
			return 'I\'m at a loss';
		}
	}();
	var _n0 = model.page;
	switch (_n0.$) {
